...
export async function renderSubpage(root, indexJson, type) {
  ...
  if (!foundType) {
    ...
    contentElement.classList.add("button-section-container");

    for (const item of content) {
      const itemElement = document.createElement("section");
      itemElement.classList.add("button-section");
    ...
    const titleElement = el("h2", { class: "subPageTitle" }, type);
    mainElement = el("main", {}, titleElement, contentElement);
  }
